...
function parseStyle(document, style, code, locationCalculator, parserOptions, cssOptions) {
    let textStart = 0;
    for (const { range, expr, exprOffset, quote, openingParenOffset, comments, } of iterateVBind(code, cssOptions)) {
        ...
        if (quote) {
            const openStart = locationCalculator.getOffsetWithGap(exprOffset - 1);
            beforeTokens.push(createSimpleToken("Punctuator", openStart, openStart + 1, quote, locationCalculator));
            const closeStart = locationCalculator.getOffsetWithGap(exprOffset + expr.length);
        ...
        try {
            const ret = parseExpression(expr, locationCalculator.getSubCalculatorShift(exprOffset), parserOptions, { allowEmpty: false, allowFilters: false });
            if (ret.expression) {
...
function* iterateVBind(code, cssOptions) {
    ...
    while ((match = re.exec(code))) {
        ...
        if (isQuote(startToken)) {
            ...
            re.lastIndex = skipComment(code, COMMENT[startToken].closing, re.lastIndex);
        }
        else {
            const openingParen = findVBindOpeningParen(code, re.lastIndex, cssOptions);
            if (!openingParen) {
                continue;
            }
            const start = match.index;
            const arg = parseVBindArg(code, openingParen.openingParenOffset + 1, cssOptions);
            if (!arg) {
                continue;
            }
            yield {
...
function findVBindOpeningParen(code, nextIndex, cssOptions) {
    ...
    while ((match = re.exec(code))) {
        ...
        if (token === "(") {
            ...
            const start = match.index;
            const end = (re.lastIndex = skipComment(code, comment.closing, re.lastIndex));
            comments.push({
                type: comment.type,
                range: [start, end],
                value: code.slice(start + token.length, end - comment.closing.length),
            });
            continue;
        }
        return null;
...
function parseVBindArg(code, nextIndex, cssOptions) {
    ...
    while ((match = re.exec(code))) {
        ...
        if (isQuote(token)) {
            ...
            if (stringRanges.length === 1) {
                ...
                const exprRange = [range[0] + 1, range[1] - 1];
                return {
...
function skipString(code, quote, nextIndex) {
...
function skipComment(code, closing, nextIndex) {
...
function skipSpaces(code, nextIndex) {
    for (let index = nextIndex; index < code.length; index++) {
        const c = code[index];
        if (c.trim()) {
            return index;
...
function parseForESLint(code, parserOptions) {
    ...
    if (!isVueFile(code, options)) {
        ...
        if ((_b = (_a = options.vueFeatures) === null || _a === void 0 ? void 0 : _a.styleCSSVariableInjection) !== null && _b !== void 0 ? _b : true) {
            ...
            parseStyleElements(styles, locationCalculator, Object.assign(Object.assign({}, options), { parser: getScriptParser(options.parser, rootAST, "template") }));
        }
